...
const Post = ({ post, type }) => {
  ...
  const [isSave , setSaved] = useState(post.saver.includes(user._id));
  ...
  return (
   ...
   <div className="w-full pb-4 drop-shadow-sm flex flex-col bg-white  border-[1px] border-gray-200 rounded-lg  ">
     ...
     <div className="flex mx-2 px-2 md:mx-4 md:px-2">
         ...
         <div className="flex ml-auto items-center justify-center ">
             ...
             <button className="p-2 text-center bg-slate-300 hover:bg-slate-400 text-gray-600 rounded-lg ml-3" onClick={() => {
               if(!isLogin.current) {
                 setShowModal(true);
               }else {
                 try{
                      if(isSave) handleUnsavePost();
                      else handleSavePost();
                 }catch(error) {
                      console.log(error);
                 }
               }
             }}>{isSave ? "Unsave" : "Save"}</button>
         </div>
       </div>
   </div>
   ...
   {showModal && <AuthModal setVisible={setShowModal}/>}
   </>
