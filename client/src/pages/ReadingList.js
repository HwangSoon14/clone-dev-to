	...
	const renderSelect = (tags) => (
      ...
      focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 focus:z-10"
		...
		>
			...
			{tags.map((tag, idx) => (
				<option key={idx} value={idx + 1}>{tag}</option>
			))}
		</select>
	);
	const renderTags = (tags) => (
		<div className='w-full h-full flex flex-col gap-y-1'>
			<button className='pl-2 py-2 w-full text-left bg-white block rounded-lg font-medium  text-purple-800'>All tags</button>
			{tags.map((tag,idx) => (
				<button className='block bg-transparent pl-2 py-2 text-gray-500 w-full text-sm text-left transition-all hover:bg-purple-100 rounded-lg'>#{tag}</button>
			))}
		</div>
	)
			   ...
			   <div className='flex items-stretch justify-between px-2 pt-4 pb-2 md:pl-6 bg-white'>
                    <div className="flex-1 flex items-stretch">
                        <div className='w-[35px] h-[auto] md:w-[40px]'>
                            <img src={post.userId.avatar} alt="avatar" className='w-full h-[35px] md:h-[40px] object-contain rounded-full'/>
                        </div>
                        <div className='flex-1 ml-2 max-w-[80%]'>
                            ...
                            <span className='inline-block text-gray-400 text-[12px] font-normal ml-1 '>•  {dayjs(post.updatedAt).format('DD-MM')}</span>

                                {post.tags.map((tag,idx) => (
                                    <span key={idx} className='text-[10px] text-gray-700 font-thin md:text-[12px] px-1 py-1 rounded-lg border-2 mx-1 border-transparent hover:bg-gray-200 hover:border-gray-300'>#{tag}</span>
                                ))} 
                            
                            </div>
                        </div>
                    </div>
                    <div className='w-[20%] text-[14px] text-gray-700 md:mt-auto '><button className='block secondary-btn ml-auto'>Archive</button></div>
                </div>
			   </Link>
            ))}
        </div>
		 ...
		 <div className="container max-w-screen-2xl mt-14 md:py-2 md:px-1 lg:px-28 bg-white-smoke  w-full overflow-x-hidden">
			<div className="px-2 py-4">
				<div className="flex flex-col md:flex-row md:items-center md:justify-between">
					...
					<div className='hidden md:block'><button className='secondary-btn'>View archive</button></div>
